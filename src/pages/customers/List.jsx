...
export const List = () => {
    ...
    return <Container className="bg-white py-3 my-3 rounded-2 shadow-sm">
        {loading ? <Loading /> : <Row>
            ...
            <Col xs="12">
                <DataTable searchables={['Name', 'Email', 'Phone', 'Address']} sortables={['Name', 'Email', 'Phone', 'Address']} data={customers.map(staff => {
                    return {
                        'Name': staff.name,
                        'Email': staff.email,
                        'Phone': staff.phone,
                        ...
                        'Actions': <>
                            <Link to={`/customers/${staff._id}`} className="btn bnt-dark btn-sm me-2">
